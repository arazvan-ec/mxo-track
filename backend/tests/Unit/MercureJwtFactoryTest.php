...
final class MercureJwtFactoryTest extends TestCase
{
    ...
    private function createTopicResolver(): TopicResolver
    {
        $routeRepo = $this->createMock(RouteRepository::class);
        $routeRepo->method('findActiveRoutePublicIdsForCustomer')->willReturn([]);
        $routeRepo->method('findActiveRoutePublicIdsForDriver')->willReturn([]);

        return new TopicResolver($routeRepo);
    }

    #[Test]
    public function createSubscriberTokenReturnsValidJwt(): void
    {
        $topicResolver = $this->createTopicResolver();

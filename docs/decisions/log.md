...
### [2026-04-01] CLAUDE.md Hierarchy — Monolithic to Directory-based

- **Problema:** CLAUDE.md monolítico de 1993 líneas se cargaba completo en cada conversación, desperdiciando ~1600 tokens de contexto en instrucciones irrelevantes para la tarea actual. El documento prescribía reglas sin explicar por qué existían, lo que degradaba la comprensión post-compactación.
- **Decisión:** Approach B+ — Reestructurar en jerarquía de CLAUDE.md por directorio, aprovechando la carga jerárquica nativa de Claude Code. Raíz (~371 líneas) contiene filosofía + workflow + reglas cross-cutting. Subdirectorios (`backend/`, `backend/src/`, `backend/tests/`, `docs/`) contienen reglas específicas de contexto. `AGENTS.md` para subagentes. `.claude/README.md` como referencia técnica del workflow engine. Aplicar 4 transformaciones: filosofía (por qué), mecánica (cómo funciona), flujo (qué alimenta qué), reglas integradas (no como secciones separadas).
- **Alternativas descartadas:** (A) Compresión simple del monolítico — no aprovecha carga jerárquica. (B) Narrativa sin jerarquía (~800 líneas) — reduce tokens pero no aprovecha mecanismo nativo. (C) Solo reorganizar sin comprimir — mejora comprensión pero no reduce tokens. (D) Plugin-first — correcto a largo plazo pero over-engineering para resolver el problema inmediato.
- **Resultado:** Implementado. Raíz 371 líneas (-81%). Contexto por tarea: pregunta simple ~371 líneas, implementación ~665 líneas. Hooks 27/27 + 19/19 core passing. 0 contenido perdido (39/39 secciones trazadas). Preparado para extracción como plugin (markers GENERIC/PROJECT-SPECIFIC) en repo `arazvan-ec/yader`.

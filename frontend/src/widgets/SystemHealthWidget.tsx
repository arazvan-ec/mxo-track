...
function ServiceCard({ name, ok, latencyMs, maxLatency, warnThreshold, icon, delay }: ServiceCardProps) {
  return (
    <div
      className="theme-card flex items-center gap-3 animate-fade-in-up"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span className="text-xs" style={{ color: ok ? 'var(--color-success)' : 'var(--color-error)' }}>
...
export function SystemHealthWidget({ data }: WidgetProps) {
  ...
  const services = [
    { name: 'PostgreSQL', ok: health.db_ok, latencyMs: live.database.latency_ms, max: 200, warn: 100, icon: <DatabaseIcon /> },
    { name: 'Redis', ok: health.redis_ok, latencyMs: live.redis.latency_ms, max: 100, warn: 50, icon: <ServerIcon /> },
    { name: 'Traccar', ok: health.traccar_ok, latencyMs: live.traccar.latency_ms, max: 1000, warn: 500, icon: <WifiIcon /> },
    { name: 'Mercure', ok: health.mercure_ok, latencyMs: live.mercure.latency_ms, max: 1000, warn: 500, icon: <BoltIcon /> },
    { name: 'OSRM', ok: health.osrm_ok, latencyMs: live.osrm.latency_ms, max: 1000, warn: 500, icon: <MapIcon /> },
    { name: 'VROOM', ok: health.vroom_ok, latencyMs: live.vroom.latency_ms, max: 2000, warn: 500, icon: <TruckIcon /> },
  ];

...
const PRESET_META: Record<ThemePreset, { label: string; colors: [string, string, string] }> = {
  default: { label: 'Default', colors: ['#0d9488', '#f8fafc', '#e2e8f0'] },
  glass: { label: 'Glass', colors: ['#0d9488', '#e8edf5', 'rgba(255,255,255,0.5)'] },
      ...
      <div className="flex gap-0.5">
        {meta.colors.map((c, i) => (
          <div
            ...
            style={{ backgroundColor: c, border: '1px solid var(--color-border)' }}
          />
        ))}
      </div>
...
interface ThemeSwitcherProps {
  /** 'inline' for embedding in TopBar, 'floating' for fixed position */
...
export function ThemeSwitcher({ mode = 'floating' }: ThemeSwitcherProps) {
  ...
  return (
    <div ref={ref} className={isInline ? 'relative' : 'fixed bottom-6 right-6 z-50'}>
      {/* Popover */}
      {open && (
        <div
          className={`absolute w-56 p-2 theme-card z-50 ${
            isInline ? 'top-full right-0 mt-2' : 'bottom-14 right-0'
          }`}
      ...
      {isInline ? (
        ...
        <button
          ...
          style={{ background: 'var(--color-accent)', color: '#fff', boxShadow: '0 4px 20px rgba(0,0,0,0.2)' }}
          title="Cambiar tema visual"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M4.098 19.902a3.75 3.75 0 005.304 0l6.401-6.402M6.75 21A3.75 3.75 0 013 17.25V4.125C3 3.504 3.504 3 4.125 3h5.25c.621 0 1.125.504 1.125 1.125v4.072M6.75 21a3.75 3.75 0 003.75-3.75V8.197M6.75 21h13.125c.621 0 1.125-.504 1.125-1.125v-5.25c0-.621-.504-1.125-1.125-1.125h-4.072M10.5 8.197l2.88-2.88c.438-.439 1.15-.439 1.59 0l3.712 3.713c.44.44.44 1.152 0 1.59l-2.879 2.88M6.75 17.25h.008v.008H6.75v-.008z" />

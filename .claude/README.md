...
| `validators/session-cut-validator.sh` | Invoked by `phase-advance.sh` on `planning → implementation` and `retrospective → finalize` | **HARD** block when `evidence.plan_session_date` (or `last_code_commit_session_date`) equals current `session_date`. Forces a fresh session for independent review at the two highest-bias transitions. Origin: 2026-04-30 cross-session resume hardening (B3). See CLAUDE.md § Context Hygiene for the antipattern that motivated the gate. | `SKIP_SESSION_CUT_GATE=1` (decision-log entry required) |

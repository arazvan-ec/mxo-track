#!/usr/bin/env bash
# Brainstorm phase validator (SOFT gate)
...
if [ -n "$ERRORS" ]; then
  echo "WARNING (SOFT): Brainstorming incompleto:"
  echo -e "$ERRORS"
  echo "Completa el brainstorming antes de continuar."
  exit 1
fi
